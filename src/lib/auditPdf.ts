...
export async function generateAuditPdf(
  ...
  // ===== Certificate Content =====
  // Title
  page.drawText('UNVERIFIED RECOVERY COPY', {
    x: margin,
    y: yPos,
    size: 18,
    font: fontBold,
  ...
  // Subtitle
  page.drawText('Recorded Activity Summary', {
    x: margin,
    y: yPos,
    size: 11,
    font: font,
  ...
  const legalText = [
    'The authoritative completed PDF was unavailable, so this recovery copy was assembled',
    'locally from stored field and activity data. It has no Somadhan Sign verification QR,',
    'is not bound to an online verification record, and must not be treated as a Certificate',
    'of Completion. Contact the document owner if an authoritative completed PDF is required.',
  ]
  ...
  for (const entry of auditEntries) {
    ...
    if (networkAddress) {
      page.drawText(`Network: ${networkAddress}`, {
        x: 400,
        y: yPos - smallLine * 2,
        size: 8,
        font: font,
  ...
  if (yPos > margin + 30) {
    ...
    yPos -= 16
    page.drawText('Unverified recovery copy. This locally assembled activity summary has no', {
      x: margin,
      y: yPos,
      size: 8,
      font: font,
    ...
    yPos -= smallLine
    page.drawText('Somadhan Sign QR verification record or authoritative completed-file hash.', {
      x: margin,
      y: yPos,
      size: 8,
      font: font,
